...
export default function ExploreScreen() {
  ...
  const searchResults = useMemo(() => {
    ...
    return (searchQ.data ?? []).map((v) =>
      visaTypeToDest(
        v,
        countries?.find(
          (c) => c.code.toLowerCase() === (v.countryCode ?? '').toLowerCase(),
        )?.name,
      ),
    );
  ...
  const cats = useMemo(
    () => ['All', ...Array.from(new Set(source.map((d) => d.cat)))],
    [source],
  );
          ...
          <View style={{ paddingHorizontal: 20, paddingBottom: 14 }}>
            ...
            >
              <View>
                ...
                <Text style={[displayText(30, 'semibold'), { marginTop: 2 }]}>
                  Where to next?
                </Text>
              </View>
              <View style={{ flexDirection: 'row', gap: 8 }}>
                ...
                <Pressable
                  onPress={() => router.push('/(explorer)/(tabs)/profile')}
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 16 }}>
          {searchMode ? (
              ...
              <View style={{ paddingTop: 50, alignItems: 'center' }}>
                {searchQ.isFetching ? (
                  ...
                  <>
                    ...
                    />
                    <Text style={{ color: EX.color.muted, fontSize: 14 }}>
                      No visas match “{query.trim()}”
                    </Text>
                  </>
                )}
              </View>
            ...
            <View style={{ paddingTop: 60, alignItems: 'center' }}>
              <ActivityIndicator color={EX.color.primary} />
              <Text
                style={{ marginTop: 12, color: EX.color.muted, fontSize: 13.5 }}
              >
                Loading visas…
              </Text>
            </View>
